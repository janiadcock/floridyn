...
class FlorisModel():
    ...
    def __init__(self, fi, turbine, index):
        self.fi = fi
        print("5 b.5", self.fi.floris.farm.wind_map.turbine_wind_speed)
...
class State():
    ...
    Args:
        ...
        observed: Whether this state is included in the state measurement or not (boolean).
        noisy: Whether this state needs to be filtered or not (boolean).
    ...
    Returns:
        FlorisModel: An instantiated FlorisModel object.
    """
    def __init__(self, name, number, method, state_type, discrete_values=None, controlled=False, observed=True, noisy=False, error_type="none", error_value=0, action_type="step", skew=0):
        self.name = name
        self.number = number
        self.method = method
        ...
        if self.action_type == "step":
            ...
            self.num_actions = 3
            self.skew = skew
        elif self.action_type == "oscillate":
            self.num_actions = 2
    ...
    def get_state(self, target=None, round_to_bin=True, count_bins=False, method=None, filter_window=None):
        ...
        if target is None:
            ...
            #NOTE temporarily commenting out this section to look for bugs
            # initialize self.val
            # if self._val is None:
            #     self._val = errored_value
            # #print("errored_value for", self.name, "and", self.number, "is", errored_value)
            # #bin_num = self.get_index(target=errored_value)

            if count_bins:
                if filter_window is None: raise ValueError("filter_window must be specified.")

                mode_bin_num = self._accumulate(self.get_index(target=errored_value), filter_window)

                if mode_bin_num is not None:
                    return self.discrete_values[mode_bin_num]
                else:
                    return None

            #     if mode_bin_num is not None:
            #         #print("Filter limit reached.")
            #         new_val = self.discrete_values[mode_bin_num]
            #         #print("Turbine", self.number, "thinks the", self.name, "is", new_val)
            #         if self.noisy and (new_val != self._val):
            #             self._target_state = new_val

            #         self._val = new_val

            #         #bin_num = mode_bin_num

            #     errored_value = self._val
    ...
    def set_state(self, agent, target_state=None):
        ...
        # NOTE: this assumes that the control_state is yaw_angle
        if agent.target_state is not None:
            target_state = agent.target_state[0]
        ...
        # NOTE: to avoid pointing at the wrong State object, just use the agent's model parameter, since it is already assumed to be yaw
...
class SimContext():
    ...
    def observe_state(self, agent, use_filter=False, method=None):
        ...
        # iterate through observable states only
        for i,state in enumerate(self.obs_states):
            state_ready = False
            # if agent is filtered, count_bins should be True in order to use the most-visisted filtering technique
            #print("Calling get_state")
            state_value = state.get_state(count_bins=state.noisy, method=method, filter_window=agent.filter_window)

            if len(state.bin_counts) == agent.filter_window:
                state_ready = True
            ...
            if state_value is not None:
                ...
                # for i,state in enumerate(self.obs_states):
                #     if state.noisy:
                #         temp_state.append(state_values[i])
                #     else:
                #         temp_state.append(agent.state[i])
            else:
    ...
    def make_states_noisy(self):
        for state in self.states:
            if state.name == "wind_speed":
                state.noisy = True
            elif state.name == "wind_direction":
                state.noisy = True
...
def _power_opt(turbine_agent):
    ...
    """
    return turbine_agent.filter_power(turbine_agent.model.turbine.power)
...
def value_function_power_opt(turbine_agent, server, time=None):
    ...
    # if turbine_agent.alias == "turbine_1":
    #     print("Value:", server.read_channel("turbine_1") + server.read_channel("turbine_2"))

    for alias in turbine_agent.neighbors:
        total_value += server.read_channel(alias)
        #print(alias, "value is read from server as", server.read_channel(alias))
    ...
    self_power = _power_opt(turbine_agent)
    total_value += self_power
    #print(turbine_agent.alias, "value is read from filter_power as", self_power)

    #print(turbine_agent.alias, "sum is", total_value)
